...
def test_discover_package_lock_npm(tmp_path: Path) -> None:
  ...
  "packages": {
    ...
    items, _warnings = discover_packages(tmp_path, {"npm"})
...
""",
    ...
    items, _warnings = discover_packages(tmp_path, {"pypi"})
    specs = {item["package_spec"] for item in items}
...
def test_discover_nested_lockfiles_with_paths_dot(tmp_path: Path) -> None:
    ...
    items, _warnings = discover_packages(tmp_path, {"npm", "pypi"}, paths=".")
    specs = {item["package_spec"] for item in items}
...
def test_discover_comma_separated_subpaths(tmp_path: Path) -> None:
    ...
    items, _warnings = discover_packages(
        tmp_path,
        {"npm", "pypi"},
...
def test_discover_dedupes_and_caps_max_items(tmp_path: Path) -> None:
    ...
    items, _warnings = discover_packages(tmp_path, {"npm", "pypi"}, max_items=80)
    specs = [item["package_spec"] for item in items]
...
def test_discover_ecosystem_filter(tmp_path: Path) -> None:
    ...
    npm_only, _ = discover_packages(tmp_path, {"npm"})
    pypi_only, _ = discover_packages(tmp_path, {"pypi"})
